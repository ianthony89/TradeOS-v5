...
**Current progress / changelog** (so you know where the build stands) → `AGENTS.md` § 2 "Recent changes (newest first)". _Last synced: 2026-06-04 — **Phase 2 CLOSED, real-portfolio UAT passed, v5 FROZEN, tagged `v5.0.0`; latest `v5.0.5` (Holdings UX reset: one unified table + upgraded Closed positions). **⚠ Migration 008 must be applied in Supabase** (holdings.exit_price + exit_date).** Repo = `github.com/ianthony89/TradeOS-v5`, deploy = `tradeos-v5.vercel.app` (NOT the legacy `TradeOS` repo). Phase status → `ROADMAP.md`; retrospective → `docs/PHASE-2-RETROSPECTIVE.md`._
