...
- **2026-06-05** — **v5.1.0 (IN REVIEW — not tagged, not frozen) — Dashboard Premium Polish, P1.** Owner-directed **visual** overhaul — *presentation only, architecture untouched* (no backend/API/DB/migration; Hub/Journal/Planner + the `position-intel`/`review-status`/`taxonomy` engine all frozen). **Core principle:** Dashboard = decision command center; eye-path must be Hero+Action → Risk → Sector/Health → Winners/Losers → Holdings — *if all cards feel equally important, it failed.* **(1) Three-tier hierarchy** (`.panel--tier1` Hero + Action Center = elevated/stronger border; tier-2 default = Risk/Sector/Health; `.panel--tier3` Winners·Losers/Holdings = flat, quiet). **(2) Hero** compact (~30% shorter, horizontal): value LEFT, **Best Position** (total return % + total P/L $) RIGHT as primary sub-info, Today Winner/Loser + Open/Closed demoted to a muted strip (replaces Largest Position). **(3) KPI** 4 cards + helper texts (Today's inline `$ ▼%` + "Today's movement"; Total/Realized/Unrealized helper lines). **(4) Action Center** = Apple-Wallet **glass** (removed `saturate`, added inset highlight + soft shadow + 3px type edge; *no glow/casino*), **urgency sort only** (EXIT→REDUCE→REVIEW→WATCH; within a tier = biggest holding first via stable sort — never drawdown/symbol/date), **two-fact reason** (`Down 95% · Review overdue 34 days` — **facts only**; *no Thesis Broken / AI Confidence* — that's Phase 3, slot reserved). **(5) Risk** = single-hue Apple **semicircle gauge** (hue = level, light→deep gradient, soft endpoint dot; *no red-yellow-green sweep*) + **natural-language drivers** ("AIXI accounts for 27% of portfolio value" — varied sentence structures, emphasized numbers, healthy variants; replaces the table + the `+22/+14` impacts). New `RiskArc` gradient + `fillTmpl` emphasis helper. EN+ZH i18n. **Docs reconciled** → *Architecture Frozen / Presentation = Premium Polish Window* (this resolves the "Dashboard FROZEN vs being changed" contradiction that confuses multi-AI). Gate green (tsc + eslint clean, build 26/26). **Browser QA still blind (tab-grouping bug) → owner screenshots are the QA loop.** **No tag, no freeze** until owner sign-off; P2 (Sector / Portfolio Health / Winners-Losers podium) + P3 (Holdings + Unrealized $) deferred to the next review round. _(in-review on `main`; see git log)_
